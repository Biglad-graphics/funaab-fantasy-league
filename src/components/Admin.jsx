...
export default function Admin() {
  ...
  const endMatch = async () => {
    ...
    // Award points based on player presence checkboxes
    for (const [playerId, presence] of Object.entries(playerPresence)) {
      if (!playerPoints[playerId]) playerPoints[playerId] = 0
      
      // If played both halves = started + played 90
      if (presence.first_half && presence.second_half) {
        playerPoints[playerId] += pointsMap.started // +1
        playerPoints[playerId] += pointsMap.played_90 // +2
      }
      // If only first half = started
      else if (presence.first_half) {
        playerPoints[playerId] += pointsMap.started // +1
      }
      // If only second half = sub appearance
      else if (presence.second_half) {
        playerPoints[playerId] += 1 // +1 for appearing
      }
    }
