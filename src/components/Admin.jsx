...
export default function Admin() {
  ...
  // CLEAN SHEET LOGIC (NEW!)
  const goalsScored = (events || []).filter(e => e.event_type === 'goal').length
  
  if (goalsScored === 0) {
    // No goals = clean sheet for all playing GK & DF
    for (const player of matchPlayers) {
      if (['GK', 'DF'].includes(player.position)) {
        ...
        if (played) {
          ...
          playerPoints[player.id] += pointsMap[`clean_sheet_${player.position}`]
        }
      }
    }
  } else {
    // Goals were scored = deduct points for each goal conceded (GK & DF only)
    for (const player of matchPlayers) {
      if (['GK', 'DF'].includes(player.position)) {
        const played = (events || []).find(e => e.player_id === player.id && ['started', 'played_90'].includes(e.event_type))
        if (played) {
  ...
  // Calculate manager points (existing logic continues...)
