...
export default function Home({ manager }) {
  ...
  const fetchData = async () => {
    const [{ data: matchData }, { data: playerData }, { data: rankData }] = await Promise.all([
    ...
    <div>
      ...
      {liveMatch && (
        <div style={styles.liveBanner}>
          ...
          <div style={styles.liveFixture}>{liveMatch.home_team} vs {liveMatch.away_team}</div>
      ...
      <div style={styles.twoCol}>
        ...
        <div style={styles.card}>
          <div style={styles.cardTitle}>📅 Upcoming Matches</div>
          {upcoming.length === 0 ? (
            <div style={styles.empty}>No upcoming matches</div>
          ) : upcoming.map(m => (
      ...
      <div style={{ ...styles.card, marginTop: '1.2rem', background: 'linear-gradient(135deg,rgba(0,230,118,.05),transparent)', border: '1px solid rgba(0,230,118,.15)' }}>
        ...
        <p style={{ color: '#5A7A5E', fontSize: '.88rem', lineHeight: 1.7 }}>
          Welcome to the FUNAAB Fantasy League 2025/26 season! Register your squad before the first matchday. Good luck! 🏆
        </p>
      </div>
    </div>
  )
...
const styles = {
  liveBanner: { background: 'linear-gradient(135deg,rgba(0,230,118,.08),transparent)', border: '1px solid rgba(0,230,118,.25)', borderRadius: '12px', padding: '1.2rem 1.5rem', marginBottom: '1.5rem', animation: 'pulse 2s infinite' },
